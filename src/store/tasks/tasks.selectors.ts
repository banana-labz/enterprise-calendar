...
const getAllTasks = createSelector(getLocalState, state => state.tasks)
...
const filterTaskNames = (tasks: TaskDTM[], searchPattern: string) => {
  ...
  return tasks.filter(
    task => task.name.toLocaleLowerCase().search(lowerCasePattern) !== -1,
  )
}
...
const getFilteredTasks = createSelector(getLocalState, state => {
...
const getEditedTask = createSelector(getLocalState, state => {
...
const getLabels = createSelector(getLocalState, state => state.labels)

const getTasksAndLabelsAsString = createSelector(getLocalState, state =>
  JSON.stringify({
